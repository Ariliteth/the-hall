...
**Last session:** Scraggle Awareness — entities witness one bar entry on emission, accumulate in `baseline-session/witnessed/{slug}` (6 slots, recency-ordered, both ends semi-flexible). Witnessed colors nudge future emission color at 25% blend. Lurk seeded as first participant: origin `[120, 65, 130]`, witnessed blend landed at `[117, 76, 115]`. New principle: Presence Has Residue.
...
**Changelog:**
- v1.000 — Scraggle Awareness: entities witness one bar entry on emission, store it in a recency-ordered slot array (6 slots, both ends semi-flexible), witnessed colors nudge future emission color at 25% blend weight. Runtime state in `baseline-session/witnessed/{slug}`. Lurk seeded as first participant. New principle: Presence Has Residue. Design doc updated.
- v0.999 — Storeroom Scraggles: three emission points (departure `🪦` weight-by-longevity, first perfect `✨` weight 0.3, trophy arrival with per-trophy color personality). `emitScraggle()` helper with extended format. Storeroom promoted to full Hall participant. Design doc updated.
...
*Lives at `scores/storeroom/`. Vanilla JS — no build step.*

A game about tending things that can't ask clearly. Objects live on shelves and express preferences through their interaction cycles — swipe, tap, hold. Each object has latent preferences it never states directly; the player infers them through repetition. Tolerance narrows over time (never widens): the longer you've known something, the more specific it becomes about what it needs. Objects pass through lifecycle states (present → developing → particular) and may depart if consistently misread after they've become particular. Departed objects leave outlines; outlines eventually produce ghosts that speak one honest sentence about their life, then dissolve. The name appears only after the ghost leaves.

Three objects: **Flicker** (tap — ceiling light, pulse count and rhythm), **Brenda** (swipe — shelf 1, speed and direction), **Glen** (hold — shelf 2, duration timing). Room-wide accuracy bar across the top. Failure celebration: shelf droop, balloons, confetti. Gimme dots absorb punishment after failures. Strict mode engages when the player is consistently accurate. Trophy system: plastic figurines arrive on shelves after interaction milestones, accumulate dust if ignored, invert (do their thing) at dust level 3, acknowledge with a tap.

Scraggle emission: departure `🪦` (amber, weight by longevity), first perfect `✨` (soft gold, weight 0.3), trophy arrival (trophy's own emoji, per-trophy color personality). `emitScraggle()` helper writes extended format to `baseline-session/scraggles`.
