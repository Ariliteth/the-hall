...
**Last session:** Storeroom Scraggles — three Scraggle emission points wired into the Storeroom. Departure emits `🪦` amber with weight proportional to `interactionCount / 40` (longer-lived objects leave a heavier mark). First perfect per object emits `✨` soft gold at weight 0.3 — fires once, quietly. Trophy arrival emits the trophy's own emoji with per-trophy color personality (teapot teal, ice cube blue, pizza orange, etc.) at weight 0.4. `emitScraggle()` helper writes extended format `{ emoji, color, weight, id, origin, ts }` to `baseline-session/scraggles`. Storeroom is now a Hall participant.
